...
namespace UserService.Tests
{
    public class AdminAddBookSelenium : IDisposable
    {
        ...
        public void AddBook_MissingRequiredFields_ShowsError()
        {
            ...
            wait.Until(d => d.FindElements(By.ClassName("error-message")).Count > 0);

            var err = driver.FindElement(By.ClassName("error-message")).Text;
            Assert.Contains("Please fill in all required fields", err);
        }
        ...
        public void AddBook_InvalidTotalCopies_ShowsError()
        {
            ...
            var err = driver.FindElement(By.ClassName("error-message")).Text;
            Assert.Contains("Total copies must be a positive number", err);
        }
